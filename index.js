...
const CHATWORK_WEBHOOK_TOKEN = process.env.CHATWORK_WEBHOOK_TOKEN; // 5fdb19e36219b23ca5d54fa880157873
...
const addMoneyWithRepay = async (aid, amount) => {
    try {
        ...
        if (p) {
            ...
            await supabase.from('players').insert({ account_id: aid, money: amount, debt: 0, slot_count: 0, work_limit: 5, msg_count: 0, slot_limit: 5, job: 'サラリーマン' });
        }
...
const checkGameProgress = async (rid) => {
    ...
    if (game.state === 'BETTING' && game.players.length >= 2 && game.players.every(p => p.bet > 0)) {
        if (game.type === 'derby') {
            ...
            game.state = 'ACTION';
            let txt = game.type === 'chouhan' ? "丁半を予想し、 [code]/chou[/code] (丁) または [code]/han[/code] (半) と発言してください。" : "親(Bot)以外の参加者は、 [code]/draw[/code] で手札を引き出してください。";
...
app.post('/webhook', (req, res) => {
    ...
    (async () => {
        try {
            ...
            // 未登録なら自動でDBへ追加 (これで/workが動かないバグが完全に解決します)
            if (!player) {
...
・ [code]/blacklist[/code], [code]/remove-rank[/code] 等[/info]`;
            ...
            if (/(^|\n)\/fi-game\b/.test(body) && gambleActive && await isUserAdmin(roomId, senderId)) {
                if (gameState[roomId] && gameState[roomId].state !== 'IDLE') {
                    ...
                    await sendTempMessage(roomId, `[info][title]⚠️ ゲーム強制終了[/title]管理者によってゲームが強制終了されました。\n(※賭け金は全額返還されました)[/info]`);
                } else {
...
募集職種は他にもあります。職業能力や転職は \`/help-gya\` をご参照ください！[/info]`;
            ...
            if ((body.trim() === '/chou' || body.trim() === '/han') && gambleActive && gameState[roomId]?.type === 'chouhan' && gameState[roomId].state === 'ACTION') {
                let pl = gameState[roomId].players.find(x => x.aid === senderId);
                if (pl && !pl.choice) {
                    pl.choice = body.trim().slice(1); await sendTempMessage(roomId, `[info][piconname:${senderId}] 「${pl.choice==='chou'?'丁':'半'}」を選択しました！[/info]`);
                    await checkGameProgress(roomId);
                }
                return;
            }
