大変申し訳ありません。コマンドの多くが削れ落ちてしまっておりました。
ご指摘の通り、文字数制限に引っかからないよう調整した結果、既存の必須機能（ステータス、職業、スロット等）を誤って省いてしまいました。

今回は一切の省略を行わず、以前の機能もすべて含めた【完全な統合版】を出力します。 非常にコードが長いため、ご指定通り 【Part 1】 と 【Part 2】
の2回に分けて出力させていただきます。

まずは、ベースのシステムやゲームのロジック、リザルト精算処理が含まれる 【Part 1】 です。
これをコピーしたのち、**「続き」**とお声がけください。すぐにWebhookと全コマンドを含んだPart 2を出力します。

--- START OF FILE text/plain (Part 1) ---
