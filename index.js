...
const updateKabuPrice = async () => {
    ...
    if (hoursPassed > 0) {
        for (let i = 0; i < hoursPassed; i++) {
            let changePercent = (Math.random() * 0.4) - 0.2; // -20% ~ +20%
            if (Math.random() < 0.05) changePercent = (Math.random() * 1.0) - 0.5; // 稀に暴落・暴騰
            
            kabuData.price += Math.floor(kabuData.price * changePercent);
            if (kabuData.price < 10) kabuData.price = 10;
    ...
    (async () => {
        try {
            ...
            if (/(^|\n)\/kabu\b/.test(body) && gambleActive) {
                ...
                return sendMessage(roomId, `[info][title]📈 株式市場 (1時間ごとに変動)[/title]💰 現在の株価: ${formatNumber(kabuData.price)} コイン\n📉 市場の残り株数: ${9999 - kabuData.totalIssued} / 9999\n📦 あなたの保有数: ${player.kabu_owned || 0} 株\n\n📊 株価推移:\n${chartUrl}[/info]`);
            }
            ...
            if (body.trim() === '/help-gya') {
                const helpMsg = `[info][title]🎰 カジノ＆ライフ 総合案内 (V49 Stock Update)[/title]
【 🏦 銀行・借金・株式 】
