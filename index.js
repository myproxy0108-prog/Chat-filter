...
// --- Chatwork メッセージ送信 (公式仕様に完全準拠) ---
const makeRp = (aid, rid, mid) => `[rp aid=${aid} to=${rid}-${mid}]`;

const sendMessage = async (rid, txt) => {
    try {
        const params = new URLSearchParams();
        params.append('body', txt);
        await cw.post(`/rooms/${rid}/messages`, params);
    } catch(e){
        console.error("sendMessage Error:", e.response ? e.response.data : e.message);
    }
};
...
const sendTempMessage = async (rid, txt, ms = 60000) => {
    try {
        const params = new URLSearchParams();
        params.append('body', txt);
        const res = await cw.post(`/rooms/${rid}/messages`, params);
        if (res && res.data && res.data.message_id) {
            setTimeout(() => cw.delete(`/rooms/${rid}/messages/${res.data.message_id}`).catch(()=>{}), ms);
        }
    } catch(e) {
        console.error("sendTempMessage Error:", e.response ? e.response.data : e.message);
    }
};
...
const generateDerby = () => {
    ...
    combos.forEach(c => {
        ...
        if (odd < 1.1) odd = 1.1; if (odd > 150) odd = 150.0; oddsMap[c.c] = Number(odd);
    });
...
const getPokerDeck = () => {
    ...
    let deck = [];
    for (let s of suits) for (let i = 0; i < rk.length; i++) deck.push({ s, r: ranks[i], v: i + 2 });
    ...
    try {
        ...
        for (let p of game.players) {
            ...
            if (comp === 0) { 
                ...
                msg += `(cracker) [piconname:${p.aid}]: [${r.hand.map(c=>c.s+c.r).join(' ')}] ${r.n} ➡ 勝ち！ (+${fNum(p.bet * r.m)} コイン)\n`; 
            } else { 
...
app.post('/webhook', (req, res) => {
    ...
    (async () => {
        try {
            ...
            // 未登録なら自動でDBへ追加 (これで/workやコマンドが動かないバグが完全に解決します)
            if (!player) {
...
【 🎰 カジノ・宝くじ 】
・ [code]/slot [掛金|max|half][/code] : スロット (1分間隔、上限99,999、1日5回)
・ [code]/buy-lot [連番|バラ] [枚数][/code] : 宝くじ購入 (1枚100コイン、0時抽選)
...
募集職種は他にもあります。職業能力や転職は \`/help-gya\` をご参照ください！[/info]`;
            ...
            if (/(^|\n)\/(catch|goal|slot-up|cm|MK|KK)\b/.test(body) && gambleActive) {
                ...
                if (cmd === 'catch') {
                    ...
                    if (Math.random() < 0.5) { cmState = { aid: senderId, expire: Date.now() + 30 * 60000 }; msg = `マスター of セレモニーオーラを展開！🔮\nここから30分間、他人がギャンブルで負けた額の50%を吸い取ります！`; } 
            ...
            if (sM && gambleActive) {
                ...
                if (bet > 0 && myMoney >= bet) {
                    ...
                    let wAmt = bet * ml; 
                    if (wA > 0) { await addMoneyWithRepay(senderId, wAmt); logBet(senderId, wAmt); }
            ...
            if (bM && gambleActive && gameState[roomId]?.state === 'BETTING') {
                ...
                if (pl && pl.bet === 0) {
                    ...
                    if (b > 0 && myMoney >= b) {
                        if (gameState[roomId].type === 'derby') {
                            let h = bM[3]; if (!h || !gameState[roomId].oddsMap[h]) return sendTempMessage(roomId, `[info]⚠️ 馬連(例: 1-2)を正しく指定してください[/info]`);
                            pl.choice = h;
                        }
