...
// --- ソロ連戦の抑制 ---
// 「部屋に他の人がいるのに一人で延々と新しいゲームを立て続ける」場合だけ、新規ゲームの開始を一時的に制限する。
// 配当自体には一切影響しない。本当に一人しかいない(直近3分に他の人の発言がない)場合は制限なし。
let recentActivity = {};   // aid -> 最終発言時刻
let soloStreak = {};       // aid -> { count, cooldownUntil } 連続ソロ開始回数
...
// ソロ連戦の追跡。ゲーム解決の直前に毎回呼ぶ(配当には一切影響しない。新規ゲーム開始の制限にのみ使う)。
const trackSoloStreak = (game) => {
    if (!game || !game.players) return;
    const real = game.players.filter(p => p.aid && p.aid !== 'bot');
    if (real.length !== 1) {
        // 複数人で遊んだ場合は連戦カウントをリセット(むしろ推奨したいので)
        for (const p of real) delete soloStreak[p.aid.toString()];
        return;
    }
    const aid = real[0].aid.toString();
    const now = Date.now();
    // 直近3分に他の人の発言があるか = 「他に遊べる人がいる状況か」を判定
    const othersAround = Object.entries(recentActivity).some(([a, ts]) => a !== aid && (now - ts) < 3 * 60 * 1000);
    if (!othersAround) {
        // 本当に一人しかいない時間帯は、何回やっても制限しない
        delete soloStreak[aid];
        return;
    }
    let s = soloStreak[aid] || { count: 0, cooldownUntil: 0 };
    s.count++;
    // 3戦連続で一人回ししたら、3分間だけ新規ゲームの開始をお休みしてもらう
    if (s.count >= 3) {
...
// 新規ゲーム開始をブロックすべきか判定する。ブロック中なら残り秒数を返す。
const getSoloStartBlockSeconds = (aid) => {
    const s = soloStreak[aid.toString()];
    if (!s || !s.cooldownUntil) return 0;
    const now = Date.now();
    const othersAround = Object.entries(recentActivity).some(([a, ts]) => a !== aid.toString() && (now - ts) < 3 * 60 * 1000);
    if (!othersAround) { s.cooldownUntil = 0; return 0; } // 一人になったなら即解除
    const remain = s.cooldownUntil - now;
    if (remain <= 0) { s.cooldownUntil = 0; return 0; }
    return Math.ceil(remain / 1000);
};

// 【Fixer】確率論的独裁: 出目・カードそのものを100%指定する。自分は最強の役、他人はゴミのような役に固定する。
const applyFixerDictatorship = (game) => {
    trackSoloStreak(game);
...
app.post('/webhook', (req, res) => {
    ...
    recentActivity[senderId] = Date.now(); // ソロ連戦判定用に「誰が今この場にいるか」を記録
...
if (localLastResetDate !== today) {
    ...
    if (!configDate || configDate.value !== today) {
        ...
        localLastResetDate = today;
            ...
            // ソロ連戦の抑制: 他に遊べる人がいるのに新しいゲームを一人で連続して立て続けている場合、
            // 少しの間だけ新規ゲームの開始をお休みしてもらう(本当に一人の時は一切制限しない)
            if (isGameCmd && !isGameCmd[0].includes('pachinko') && gambleActive) {
                let blockSec = getSoloStartBlockSeconds(senderId);
                if (blockSec > 0) {
                    return sendTempMessage(roomId, `[info]⚠️ [piconname:${senderId}]\n一人での連戦が続いています。少し休むか、誰かと一緒に遊んでください。\n(残り ${blockSec} 秒)[/info]`);
                }
            }
...
/#take [金], /#fi-game, /#st-gya, /#fi-gya, /#blacklist 等[/info]`;
            ...
            const cJobMatch = body.match(/(^|\n)[/#]job\s+(サラリーマン|公務員|警察官|プロスポーツ選手|賭博師|ギャンブルオーナー|未来人|逆転のギャンブラー|銀行員|大富豪の執事|賞金稼ぎ|数学者|パチプロ|てんびん|隻眼|トレーダー|保険屋|カジノ王|占い師|シェフ|エボリューショナー|Fixer|Terminal|Photographer)/);
            if (cJobMatch && gambleActive) {
                const jn = cJobMatch[2]; const cs = {
                    ...
                    'Terminal': 3000000, 'Photographer': 4000000
                };
...
[hr]※転職コマンド: /#job 役職名[/info]`);
            ...
            if (/(^|\n)[/#]work\b/.test(body) && gambleActive) {
                ...
                else if(myJob === 'Terminal'){ e=Math.floor(Math.random()*2501)+2000; m=`不正アクセスで裏帳簿を書き換え、 ${formatNumber(e)} コイン稼ぎました！🖥️`; }
                else if(myJob === 'Photographer'){ e=Math.floor(Math.random()*2501)+2000; m=`決定的瞬間を切り取って売り、 ${formatNumber(e)} コイン稼ぎました！📷`; }
                // アカウント単位でロック＋再取得し、job_stateへの競合書き込みによる実績の取りこぼしを防止
                let workTotal = 0;
                await withBadgeLock(senderId, async () => {
