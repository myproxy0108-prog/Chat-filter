...
// --- API Client Init ---
const cw = axios.create({ baseURL: 'https://api.chatwork.com/v2', headers: { 'X-ChatWorkToken': process.env.CHATWORK_API_TOKEN } });
...
const processedMsgIds = new Set(); // タイムアウト連投(再送)防止用キャッシュ

let betLogs = [];   // /KK (タイムトラベル) 用の過去5分ログ
let cmState = null; // /cm (マスター) の吸収状態
let mkState = null; // /MK (タイムトラベラー) のイカサマ状態
...
const verifySignature = (req) => {
    ...
    if (!sig || !req.rawBody || !process.env.CHATWORK_WEBHOOK_TOKEN) return false;
...
const sendMessage = async (rid, txt) => {
    try {
        ...
        await cw.post(`/rooms/${rid}/messages`, p, { headers: { 'Content-Type': 'application/x-www-form-urlencoded' } });
...
const sendTempMessage = async (rid, txt, ms = 60000) => {
    try {
        const p = new URLSearchParams(); p.append('body', txt);
        const res = await cw.post(`/rooms/${rid}/messages`, p, { headers: { 'Content-Type': 'application/x-www-form-urlencoded' } });
        if (res && res.data && res.data.message_id) {
            setTimeout(() => cw.delete(`/rooms/${rid}/messages/${res.data.message_id}`).catch(()=>{}), ms);
        }
    } catch(e) { console.error("sendTempMessage Error:", e.response?.data || e.message); }
};
...
const addMoneyWithRepay = async (aid, amount) => {
    try {
        ...
        else await supabase.from('players').insert({ account_id: aid, money, debt, slot_count: 0, work_limit: 5, msg_count: 0, slot_limit: 5, job: 'サラリーマン' });
        
        logBet(aid, actualDiff);
    } catch (e) { console.error("addMoneyWithRepay Error:", e.message); }
};
...
const kickTarget = async (rid, targetAids, action = 'readonly') => {
    try {
        ...
        await cw.put(`/rooms/${rid}/members`, p.toString(), { headers: { 'Content-Type': 'application/x-www-form-urlencoded' } });
    } catch(e) {}
};
...
const evalPoker = (cards) => {
    ...
    let rank = 1, tieBreak = [];
    if (isFlush && isStraight) { if (straightHigh === 14) { rank = 10; tieBreak = [14]; } else { rank = 9; tieBreak = [straightHigh]; } } 
...
const drawPoker = (deck) => { let hand = []; for(let i=0; i<5; i++) hand.push(deck.pop()); return { hand, ...evalPoker(hand) }; };

// --- ゲーム進行・タイマー (サーバーレス対応ハイブリッド型) ---
const startGameTimer = (rid, ms = 60000, isDerby = false) => {
    let game = gameState[rid]; if (!game) return;
    
    game.expireAt = Date.now() + ms; // Vercel対策の絶対時刻記録
    game.remindAt = isDerby ? Date.now() + (ms - 60000) : null;
...
const handleGameTimeout = async (rid) => {
    ...
    if (game.state === 'RECRUITING') {
        ...
        for (let p of game.players) {
            ...
            if (game.state === 'ACTION' && (game.type === 'chouhan' && !p.choice || game.type === 'poker' && !p.res && p.aid !== game.host)) isKicked = true;
            
            if (isKicked) { kicked.push(p.aid); if (p.bet > 0) await addMoneyWithRepay(p.aid, p.bet); } 
...
// --- メイン Webhook 処理 ---
const processedMsgIds = new Set(); // Chatworkの連投バグ防止
...
app.post('/webhook', async (req, res) => {
    ...
    if (!ev || ev.webhook_event_type !== 'message_created') return res.status(200).send('OK');

    // 連投再送バグ防止
    if (processedMsgIds.has(ev.message_id)) return res.status(200).send('OK');
    processedMsgIds.add(ev.message_id);
    ...
    try {
        // --- サーバーレス対策：チャット受信時にタイムアウトチェックを走らせる ---
        for (let r in gameState) {
            let g = gameState[r];
            if (g && g.state !== 'IDLE') {
                let now = Date.now();
                if (g.remindAt && now > g.remindAt) { g.remindAt = null; if (g.state === 'BETTING') await sendTempMessage(r, `[info]⏳ 競馬のベット締め切りまで【残り1分】です！\nまだの方は [code]/bet [額] [馬1-馬2][/code] を入力してください。[/info]`); }
                if (g.expireAt && now > g.expireAt) { g.expireAt = null; await handleGameTimeout(r); }
            }
        }
        ...
        if (isBanned) { 
            ...
            await cw.delete(`/rooms/${roomId}/messages/${ev.message_id}`).catch(()=>{}); 
        ...
        let myMoney = player.money || 0, myDebt = player.debt || 0, myJob = player.job || 'サラリーマン';
...
・ [code]/blacklist[/code], [code]/remove-rank[/code] 等[/info]`;
        ...
        if (/(^|\n)\/(blacklist|reblacklist|remove-rank)\b/.test(body) && await isUserAdmin(roomId, senderId)) {
            ...
            if (!targetAid && cmd !== 'add') return res.status(200).send('OK'); 
            ...
            if (cmd === 'rank') {
                ...
                if (ex.includes(targetAid)) { ex = ex.filter(i => i !== targetAid); await sendTempMessage(roomId, `[info][title]設定完了[/title][piconname:${targetAid}] 様のランキング除外を解除しました。[/info]`); } 
                else { ex.push(targetAid); await sendTempMessage(roomId, `[info][title]設定完了[/title][piconname:${targetAid}] 様をランキングから除外しました。[/info]`); }
                await supabase.from('config').upsert({key:'rank_excluded', value:JSON.stringify(ex)});
            } else if (cmd === 'add') { 
                await supabase.from('blacklist').insert({account_id: targetAid}); await kickTarget(roomId, [targetAid], 'readonly'); 
                await sendTempMessage(roomId, `[info][title]🚫 追放完了[/title][piconname:${targetAid}] をブラックリストに登録し、権限を「閲覧のみ」に変更しました。[/info]`); 
            } else if (cmd === 'remove') { 
                await supabase.from('blacklist').delete().eq('account_id', targetAid); await sendTempMessage(roomId, `[info][title]✅ 解除完了[/title][piconname:${targetAid}] の追放状態を解除しました。[/info]`); 
            } else if (cmd === 'list') { 
        ...
        if (/(^|\n)\/omikuji\b/.test(body) && gambleActive) {
            if (player && player.omikuji_date === today) {
                await sendTempMessage(roomId, `[info][title]⚠️ おみくじ[/title]${makeRp(senderId, roomId, ev.message_id)}\n本日のおみくじは既に引いています。\n(結果: ${player.omikuji_result})[/info]`);
                return res.status(200).send('OK');
            }
            let r = Math.random() * 100, resStr = "", eff = "";
            if(r < 10) { resStr = "大吉"; eff = "(cracker) スロット確率が【大幅UP (0.5%)】！"; } 
            else if(r < 30) { resStr = "中吉"; eff = "(cracker) スロット確率が【少しUP (0.3%)】！"; } 
            else if(r < 60) { resStr = "小吉"; eff = "🎯 スロット確率は通常通り(0.1%)です。"; } 
            else if(r < 85) { resStr = "吉"; eff = "🎯 スロット確率は通常通り(0.1%)です。"; } 
            else if(r < 95) { resStr = "凶"; eff = "💧 スロット確率が【少しDOWN】..."; } 
            else { resStr = "大凶"; eff = "💀 スロット確率が【大幅DOWN (0.01%)】..."; }
            
            await supabase.from('players').update({ omikuji_date: today, omikuji_result: resStr }).eq('account_id', senderId);
            await sendMessage(roomId, `[info][title]⛩️ おみくじ結果[/title]${makeRp(senderId, roomId, ev.message_id)}\n[hr]今日の運勢は...【 ${resStr} 】です！\n\n${eff}[/info]`);
            return res.status(200).send('OK');
        }
        ...
        if (debtMatch && gambleActive) {
            ...
            if (amt > 0) {
                if (currentMonthlyDebt + amt > 5000) {
                    await sendTempMessage(roomId, `[info][title]⚠️ 借金上限エラー[/title]${makeRp(senderId, roomId, ev.message_id)}\n1ヶ月の借金上限(5000)を超過します！\n(今月は既に ${currentMonthlyDebt} コイン借りています)[/info]`);
                } else {
        ...
        if (/(^|\n)\/give/.test(body) && gambleActive) {
            ...
            if (targetAid && amt > 0) {
                ...
                if (av < amt) {
                    await sendTempMessage(roomId, `[info][title]⚠️ 送金エラー[/title]${makeRp(senderId, roomId, ev.message_id)}\n送金枠(純資産)が不足しています！\n(送金可能額: ${fNum(av)} コイン)[/info]`);
                } else {
        ...
        if (cJobMatch && gambleActive) {
            ...
            if (myJob === jn) await sendTempMessage(roomId, `[info]⚠️ ${makeRp(senderId, roomId, ev.message_id)}\nすでに ${jn} に就いています！[/info]`);
            else if (myMoney < cs[jn]) await sendTempMessage(roomId, `[info]⚠️ ${makeRp(senderId, roomId, ev.message_id)}\nお金が足りません！(転職費用: ${fNum(cs[jn])} コイン)[/info]`);
            else {
...
[hr]※転職コマンド: [code]/job 役職名[/code][/info]`;
        ...
        if (/(^|\n)\/work\b/.test(body) && gambleActive) {
            if (player.work_limit <= 0) await sendTempMessage(roomId, `[info]⚠️ ${makeRp(senderId, roomId, ev.message_id)}\n本日の仕事回数が上限(5回)に達しました。[/info]`);
            else if (Date.now() - (player.last_work_time || 0) < 600000) await sendTempMessage(roomId, `[info]⚠️ ${makeRp(senderId, roomId, ev.message_id)}\n休憩中です！仕事は10分間隔で行えます。[/info]`);
            else {
                let e = 0, m = "";
        ...
        if (/(^|\n)\/(catch|goal|slot-up|cm|MK|KK)\b/.test(body) && gambleActive) {
            ...
            if (cmd === 'catch' && myJob !== '警察官') return res.status(200).send('OK');
            if (cmd === 'goal' && myJob !== 'プロスポーツ選手') return res.status(200).send('OK');
            if (cmd === 'slot-up' && myJob !== '賭博師') return res.status(200).send('OK');
            if (cmd === 'cm' && myJob !== 'マスター') return res.status(200).send('OK');
            if ((cmd === 'KK' || cmd === 'MK') && myJob !== 'タイムトラベラー') return res.status(200).send('OK');

            if (player.skill_date === today) { await sendTempMessage(roomId, `[info]⚠️ 今日の特殊能力はすでに使用済みです！[/info]`); return res.status(200).send('OK'); }
            
            let msg = "";
            if (cmd === 'catch') {
                if (Math.random() < 0.3) { await addMoneyWithRepay(senderId, 800); msg = `犯人を逮捕しました！特別報酬 800 コイン獲得！🚨`; } else msg = `犯人を逃してしまいました...🏃‍♂️💨`;
            } else if (cmd === 'goal') {
                if (Math.random() < 0.3) { await addMoneyWithRepay(senderId, 1000); msg = `スーパーゴールを決めました！特別報酬 1000 コイン獲得！🥅✨`; } else msg = `シュートは外れてしまいました...🤦‍♂️`;
            } else if (cmd === 'slot-up') {
                let newLimit = Math.floor(Math.random() * 6) + 10; await supabase.from('players').update({ slot_limit: newLimit }).eq('account_id', senderId);
                ...
                for (let a in diffs) { if (diffs[a] !== 0) await addMoneyWithRepay(a, diffs[a]); }
        ...
        const sM = body.match(/(^|\n)\/slot\s+(max|half|[0-9]+)/);
        if (sM && gambleActive) {
            if (player.slot_count >= mySlotLimit) { await sendTempMessage(roomId, `[info]⚠️ ${makeRp(senderId, roomId, ev.message_id)}\n本日のスロットは上限(${mySlotLimit}回)に達しました！[/info]`); return res.status(200).send('OK'); }
            if (Date.now() - Number(player.last_slot_time || 0) < 120000) { await sendTempMessage(roomId, `[info]⚠️ ${makeRp(senderId, roomId, ev.message_id)}\nスロット休憩中(2分間隔)です！[/info]`); return res.status(200).send('OK'); }
            ...
            if (bet > 0 && myMoney >= bet) {
                ...
                let ml = 0, sy = "", resMsg = "";
                if(r < 0.1){ ml=30; sy="🐉 | 🐉 | 🐉"; resMsg="🔥 超大当たり！！！ (30倍) 🔥"; } else if(r < 3.1){ ml=10; sy="7️⃣ | 7️⃣ | 7️⃣"; resMsg="✨ 大当たり！ (10倍) ✨"; } else if(r < 9.1){ ml=3; let s=["6️⃣","5️⃣","4️⃣"][Math.floor(Math.random()*3)]; sy=`${s} | ${s} | ${s}`; resMsg="(cracker) 当たり！ (3倍)"; } else if(r < 19.1){ ml=2; let s=["3️⃣","2️⃣","1️⃣"][Math.floor(Math.random()*3)]; sy=`${s} | ${s} | ${s}`; resMsg="(cracker) 当たり！ (2倍)"; } else if(r < 29.1){ ml=2; let s=["🍉","🍋","🔔","🍇"][Math.floor(Math.random()*4)]; sy=`${s} | ${s} | ${s}`; resMsg="🍇 フルーツ揃い！ (2倍)"; } else if(r < 49.1){ ml=2; let o=["🍉","🍋","🔔","🍇","7️⃣","6️⃣","5️⃣"]; let s1=o[Math.floor(Math.random()*o.length)], s2=o[Math.floor(Math.random()*o.length)]; let a=["🍒",s1,s2].sort(()=>Math.random()-0.5); sy=a.join(" | "); resMsg="🍒 チェリー出現！ (2倍)"; } else { ml=0; let o=["🍉","🍋","🔔","🍇","7️⃣","6️⃣","5️⃣"]; let r1=o[Math.floor(Math.random()*o.length)], r2=o[Math.floor(Math.random()*o.length)], r3=o[Math.floor(Math.random()*o.length)]; while(r1===r2&&r2===r3) r3=o[Math.floor(Math.random()*o.length)]; sy=`${r1} | ${r2} | ${r3}`; resMsg="💀 はずれ..."; }
                
                let wA = bet * ml; 
                if (wA > 0) { await addMoneyWithRepay(senderId, wA); } else { await applyCM(bet); } 
                await sendMessage(roomId, `[info][title]🎰 SLOT MACHINE ${oM}[/title]${makeRp(senderId, roomId, ev.message_id)}\n[hr]　▶ [ ${sy} ] ◀　\n[hr]${resMsg}\n\n賭け金: ${fNum(bet)} ➡ 獲得: ${fNum(wA)} コイン\n(残り回数: ${mySlotLimit - (player.slot_count + 1)}回)[/info]`);
            } else await sendTempMessage(roomId, `[info]⚠️ ${makeRp(senderId, roomId, ev.message_id)} お金が足りません！[/info]`);
            return res.status(200).send('OK');
        }
        ...
        if (bM && gambleActive && gameState[roomId]?.state === 'BETTING') {
            ...
            if (pl && pl.bet === 0) {
                ...
                if (b > 0 && myMoney >= b) {
                    if (gameState[roomId].type === 'derby') {
                        let h = bM[3]; if (!h || !gameState[roomId].oddsMap[h]) return sendTempMessage(roomId, `[info]⚠️ 馬連(例: 1-2)を正しく指定してください\n例: [code]/bet 100 1-2[/code][/info]`);
                        pl.choice = h;
                    }
        ...
        if ((body.trim() === '/chou' || body.trim() === '/han') && gambleActive && gameState[roomId]?.type === 'chouhan' && gameState[roomId].state === 'ACTION') {
            let pl = gameState[roomId].players.find(x => x.aid === senderId);
            if (pl && !pl.choice) { 
                pl.choice = body.trim().slice(1); await sendTempMessage(roomId, `[info]🎯 [piconname:${senderId}] 「${pl.choice==='chou'?'丁(偶数)':'半(奇数)'}」を選択しました！[/info]`); 
                await checkGameProgress(roomId); 
            }
            return res.status(200).send('OK');
        }
        ...
        if (body.trim() === '/draw' && gambleActive && gameState[roomId]?.type === 'poker' && gameState[roomId].state === 'ACTION') {
            ...
            if (pl && !pl.res && senderId !== gameState[roomId].host) {
                ...
                await sendMessage(roomId, `[info]🃏 [piconname:${senderId}] の役: ${pl.res.n} [ ${pl.res.hand.map(c=>c.s+c.r).join(' ')} ][/info]`); 
                await checkGameProgress(roomId);
            }
            return res.status(200).send('OK');
        }
