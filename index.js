...
let ownerSkill = { aid: null, expire: 0, evoBoost: false };
...
const processBuffs = async (aid, isWin, isLose, isDraw, mult, resTxt, betAmount = 0) => {
    ...
    if (isWin && js.sekigan_active) {
        ...
        if (js.evo_boost_sekigan) {
            js.evo_boost_sekigan = false;
            mult += 2.0;
            resTxt += `👁️‍🗨️ 【隻眼】(エボリューション強化)の効果で配当が大幅アップ！ `;
        } else {
            mult += 1.0; 
        ...
        js.sekigan_active = false; updated = true;
        if (js.evo_boost_sekigan) js.evo_boost_sekigan = false;
    }
...
const processOwnerSkill = async (loserAid, lostAmount, roomId) => {
    ...
    if (ownerSkill.expire > now && ownerSkill.aid && ownerSkill.aid !== loserAid.toString()) {
        ...
        let chance = ownerSkill.evoBoost ? 0.7 : 0.5;
        let pct = ownerSkill.evoBoost ? 0.7 : 0.5;
        if (Math.random() < chance) { 
            let stealAmount = Math.floor(lostAmount * pct); 
            if (stealAmount > 0) {
...
const processBounty = async (loserAid, lostAmount, roomId) => {
    ...
    if (hunters) {
        for (let h of hunters) {
            ...
            if (js.bounty_target === loserAid.toString() && !js.daily_bounty_used) {
                let pct = js.evo_boost_bounty ? 0.2 : 0.1;
                let reward = Math.floor(lostAmount * pct);
                if (reward > 0) {
                    await addMoney(h.account_id, reward);
                    js.daily_bounty_used = true;
                    if (js.evo_boost_bounty) js.evo_boost_bounty = false;
                    await supabase.from('players').update({ job_state: JSON.stringify(js) }).eq('account_id', h.account_id);
...
const resolveChouhan = async (roomId, mId) => {
    ...
    for (let player of game.players) {
        ...
        if (js.tenbin_active && game.players.length >= 2) {
            ...
            let tenbinEvoBoost = !!js.evo_boost_tenbin;

            if (tenbinEvoBoost) js.evo_boost_tenbin = false;
            ...
            if (Math.random() < 0.1) {
                ...
                let prob = tenbinEvoBoost ? (0.5 + Math.random() * 0.2) : (0.3 + Math.random() * 0.2);
                if (Math.random() < prob) {
    ...
    if (!configDate || configDate.value !== today) {
        ...
        localLastResetDate = today;
            ...
            if (player && player.last_daily_date !== today) {
                ...
                player.job_state.evo_boost_future = false;
                player.job_state.evo_boost_owner = false;
                player.job_state.evo_boost_tenbin = false;
                player.job_state.evo_boost_sekigan = false;
                player.job_state.evo_boost_bounty = false;
...
あっ！中身は空っぽだった！詐欺られた！ (-${formatNumber(cost)} コイン)[/info]`);
            ...
            if (/(^|\n)[/#]next-future\b/.test(body) && gambleActive) {
                ...
                let futureMaxUses = player.job_state.evo_boost_future ? 3 : 1;
...
🧬 エボリューショナー (費用: 50,000,000 / 実績「完全制覇」10個以上で解禁)\n ▶ /#evolve [役職1] [役職2] (1日1回、既存の役職2つの能力を掛け合わせた特殊能力を得る)
[hr]※転職コマンド: /#job 役職名[/info]`);
            ...
            if (evolveMatch && gambleActive) {
                ...
                // アクティブ系(コマンドを強化): 既存のコマンドの効果が本日1回に限り強化される
                const EVO_ACTIVE_BOOSTS = {
                    '未来人': { flag: 'evo_boost_future', desc: '/#next-future の使用可能回数が1回→3回に強化' },
                    'ギャンブルオーナー': { flag: 'evo_boost_owner', desc: '/#owner の発動時間が30分→60分、回収率が50%→70%に強化' },
                    'てんびん': { flag: 'evo_boost_tenbin', desc: '/#tenbin の勝率上昇量が強化される' },
                    '隻眼': { flag: 'evo_boost_sekigan', desc: '/#sekigan の配当上昇が+1.0倍→+2.0倍に強化' },
                    '賞金稼ぎ': { flag: 'evo_boost_bounty', desc: '/#bounty の奪取率が10%→20%に強化' },
                };
                ...
                ts.evo_combo = [j1, j2];
                let resultMsg = `[info][title]🧬 エボリューション発動！[/title][piconname:${senderId}]\n「${j1}」×「${j2}」の力を掛け合わせた！`;
                ...
                for (let j of [j1, j2]) {
                    ...
                    if (EVO_ACTIVE_BOOSTS[j]) {
                        ts[EVO_ACTIVE_BOOSTS[j].flag] = true;
                        resultMsg += `\n⚡ ${EVO_ACTIVE_BOOSTS[j].desc} (本日1回限り)`;
                    }
                }
            ...
            if (/(^|\n)[/#]owner\b/.test(body) && gambleActive) {
                ...
                let ownerBoosted = !!player.job_state.evo_boost_owner;
                ownerSkill.aid = senderId;
                ownerSkill.expire = now + (ownerBoosted ? 60 : 30) * 60 * 1000;
                ...
                return sendTempMessage(roomId, `[info][title]👑 オーナー権限発動[/title][piconname:${senderId}]\nここから${ownerBoosted ? 60 : 30}分間、他人がギャンブルで負けた金額の${ownerBoosted ? 70 : 50}%を${ownerBoosted ? 70 : 50}%の確率で回収します...！${ownerBoosted ? ' (エボリューション強化！)' : ''}[/info]`);
            }
