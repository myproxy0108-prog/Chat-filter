...
const kickTarget = async (rid, targetAids, action = 'readonly') => {
    try {
        ...
        let admins = membersList.filter(m => m.role.match(/admin|creator/)).map(m => m.account_id.toString());
...
const resolveChinchiro = async (rid) => {
    ...
    for (let p of game.players) {
        ...
        let win = (r.r > parentRoll.r) || (r.r === parentRoll.r && r.s > parentRoll.s), draw = (r.r === parentRoll.r && r.s === parentRoll.s);
...
app.post('/webhook', async (req, res) => {
    ...
    const roomId = ev.room_id, body = ev.body || "", senderId = ev.account_id.toString(), msgId = ev.message_id, today = getTodayStr(), thisMonth = getThisMonthStr();

    (async () => {
        try {
            const rpMatch = body.match(/\[(?:rp|返信|qtmeta|reply)\s+aid=([0-9]+)/i); const repliedAid = rpMatch ? rpMatch[1] : null;
            ...
            let myMoney = player ? player.money : 0, myDebt = player ? (player.debt || 0) : 0, myJob = player ? (player.job || 'サラリーマン') : 'サラリーマン', currentMonthlyDebt = (player && player.debt_month === thisMonth) ? (player.monthly_debt || 0) : 0, mySlotLimit = player ? (player.slot_limit || 5) : 5;
...
・ [code]/blacklist[/code], [code]/remove-rank[/code] 等[/info]`;
            ...
            if (/(^|\n)\/take\b/.test(body) && gambleActive && await isUserAdmin(roomId, senderId)) {
                let amt = parseInt((body.match(/(^|\n)\/take\s+([0-9]+)/)||[])[2] || (body.match(/(^|\n)\/take\s+[0-9]+\s+([0-9]+)/)||[])[3], 10), targetAid = repliedAid || (body.match(/(^|\n)\/take\s+([0-9]+)\s+[0-9]+/) || [])[2];
            ...
            if (/(^|\n)\/(blacklist|reblacklist|remove-rank)\b/.test(body) && await isUserAdmin(roomId, senderId)) {
                let targetAid = repliedAid || (body.match(/(^|\n)\/(?:blacklist|reblacklist|remove-rank)\s+([0-9]+)/) || [])[2], cmd = body.includes('/remove-rank') ? 'rank' : (body.includes('/reblacklist') ? 'remove' : 'add');
                ...
                if (cmd === 'add') { await supabase.from('blacklist').insert({account_id: targetAid}); await kickTarget(roomId, [targetAid], 'readonly'); return sendTempMessage(roomId, `[info][title]🚫 追放完了[/title][piconname:${targetAid}] をBL登録し、権限を「閲覧のみ」に変更しました。[/info]`); }
...
[hr]※転職コマンド: [code]/job 役職名[/code][/info]`);
            ...
            if (/(^|\n)\/(catch|goal|slot-up|cm|MK|KK)\b/.test(body) && gambleActive && player) {
                ...
                let msg = "";
                if (cmd === 'catch') { if (Math.random() < 0.3) { await addMoneyWithRepay(senderId, 800); msg = `犯人を逮捕しました！特別報酬 800 コイン獲得！🚨`; } else msg = `犯人を逃してしまいました...🏃‍♂️💨`; } 
                else if (cmd === 'goal') { if (Math.random() < 0.3) { await addMoneyWithRepay(senderId, 1000); msg = `スーパーゴールを決めました！特別報酬 1000 コイン獲得！🥅✨`; } else msg = `シュートは外れてしまいました...🤦‍♂️`; } 
                else if (cmd === 'slot-up') { let newLimit = Math.floor(Math.random() * 6) + 10; await supabase.from('players').update({ slot_limit: newLimit }).eq('account_id', senderId); msg = `ギャンブル魂が燃え上がった！🔥 本日のスロット上限が ${newLimit} 回にアップしました！`; } 
                else if (cmd === 'cm') { if (Math.random() < 0.5) { cmState = { aid: senderId, expire: Date.now() + 30 * 60000 }; msg = `マスターのオーラを展開！🔮\nここから30分間、他人がギャンブルで負けた額の50%を吸収します！`; } else msg = `オーラの展開に失敗しました...今日はもう使えません。💦`; } 
                else if (cmd === 'MK') { mkState = { aid: senderId }; msg = `未来予知完了...👁️✨\n次に行われるゲームで、あなたに80%の確率で「奇跡」が起こります！`; } 
                else if (cmd === 'KK') {
            ...
            if (body.match(/(^|\n)\/(chouhan|cc|derby)\b/) && gambleActive) {
                ...
                let t = body.includes('/derby') ? 'db' : (body.includes('/cc') ? 'cc' : 'chouhan');
                ...
                if (t === 'db') { let dO = generateDerby(); gameState[roomId].oddsMap = dO.oddsMap; gameState[roomId].oddsStr = dO.oddsStr; gameState[roomId].st = dO.stats; }
                
                sendTempMessage(roomId, `[info][title]${tN} 募集開始[/title]ホスト: [piconname:${senderId}]\n\n参加者は ${ex} と入力！(現在 1人)\n[hr]ホストが [code]/start${t==='chouhan'?'chouhan':t}[/code] と打つとゲームが進行します。[/info]`); 
                return;
            }
...
if (process.env.NODE_ENV !== 'production') app.listen(process.env.PORT || 3000, () => console.log(`Live V41 FINAL`));
module.exports = app;
