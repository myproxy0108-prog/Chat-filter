...
const spamRecords = {};
const gameState = {}; // ゲーム状態を一元管理する変数
...
const addMoney = async (aid, amount) => {
    ...
    if (p) {
        ...
        await supabase.from('players').insert({ account_id: aid, money, debt, slot_count: 0, work_limit: 5, msg_count: 0 });
    }
};
...
const checkGameProgress = async (rid) => {
    ...
    if (game.state === 'BETTING' && game.players.length >= 2 && game.players.every(p => p.bet > 0)) {
        if (game.type === 'derby') {
            ...
            await sendTempMessage(rid, `[info][title]🎲 進行フェーズ[/title]全員のベットが完了しました！\n${txt}\n[hr](※制限時間: 1分)[/info]`);
            startGameTimer(rid, 60000);
        }
...
const resolveChinchiro = async (rid) => {
    ...
    for (let p of game.players) {
        ...
        if (draw) { 
            ...
            msg += `🎉 [piconname:${p.aid}]: [${r.d.join('')}] ${r.n} ➡ 勝ち！ (+${fNum(p.bet * mult)})\n`; 
...
const resolveChouhan = async (rid) => {
    ...
    for (let p of game.players) {
        if (p.choice === result) { await addMoney(p.aid, p.bet * 2); msg += `🎉 [piconname:${p.aid}]: 的中！ (+${fNum(p.bet * 2)} コイン)\n`; } 
    ...
    (async () => {
        try {
                ...
                if (!configDate || configDate.value !== today) {
                    ...
                    if (tickets.length > 0) {
                        ...
                        if (winners.length > 0) {
                            ...
                            for (let w of winners.slice(0, 20)) resetMsg += `🎉 [piconname:${w.a}]: 予想[${w.num}] ➡ ${w.name} (+${fNum(w.p)} コイン)\n`;
            ...
            if (!player && gambleActive && !body.startsWith('/')) {
                await supabase.from('players').insert({ account_id: senderId, money: 0, debt: 0, work_limit: 5, msg_count: 1 });
            ...
            if (body.trim() === '/help-gya') {
                const h = `[info][title]🎰 カジノ＆ライフ 総合案内 (V37 FINAL)[/title]
【 🏦 銀行・ステータス 】
...
・ [code]/blacklist[/code], [code]/remove-rank[/code] 等[/info]`;
            ...
            if (debtMatch && gambleActive) {
                ...
                if (amt > 0) {
                    if (currentMonthlyDebt + amt > 5000) return sendTempMessage(roomId, `[info][title]⚠️ 借金上限[/title]${makeRp(senderId, roomId, msgId)}\n1ヶ月の借金上限(5000)を超過します！\n(今月は既に ${cDebt} コイン借りています)[/info]`);
            ...
            if (body.trim() === '/status') {
                if (player) {
                    const remSlot = Math.max(0, 3 - player.slot_count);
                    const dStr = myDebt > 0 ? `\n💳 借金: -${fNum(myDebt)} コイン` : '';
            ...
            if (jM && gambleActive) {
                ...
                if (myJob === jn) return sendTempMessage(roomId, `[info]⚠️ ${mkRp(senderId, roomId, msgId)}\nすでに ${jn} に就いています！[/info]`);
                if (myMoney < cs[jn]) return sendTempMessage(roomId, `[info]⚠️ ${mkRp(senderId, roomId, msgId)}\nお金が足りません！(費用: ${fNum(cs[jn])} コイン)[/info]`);
...
[hr]※転職コマンド: [code]/job 役職名[/code][/info]`);
            ...
            if (/(^|\n)\/work\b/.test(body) && gambleActive && player) {
                if (player.work_limit <= 0) return sendTempMessage(roomId, `[info]⚠️ ${mkRp(senderId, roomId, msgId)}\n本日の仕事回数が上限(5回)に達しました。[/info]`);
                if (Date.now() - (player.last_work_time || 0) < 600000) return sendTempMessage(roomId, `[info]⚠️ ${mkRp(senderId, roomId, msgId)}\n休憩中です！仕事は10分間隔で行えます。[/info]`);
            ...
            if ((/(^|\n)\/catch\b/.test(body) || /(^|\n)\/goal\b/.test(body)) && gambleActive && player) {
                let iC = /(^|\n)\/catch\b/.test(body);
            ...
            if (sM && gambleActive && player) {
                if (player.slot_count >= 3) return sendTempMessage(roomId, `[info]⚠️ ${mkRp(senderId, roomId, msgId)}\n本日のスロットは上限(1日3回)に達しました！[/info]`);
                if (Date.now() - Number(player.last_slot_time || 0) < 600000) return sendTempMessage(roomId, `[info]⚠️ ${mkRp(senderId, roomId, msgId)}\nスロット休憩中(10分間隔)です！[/info]`);
            ...
            if (body.match(/(^|\n)\/(chouhan|cc|derby)\b/) && gambleActive) {
                ...
                gameState[roomId] = { type: t, state: 'REC', host: sId, players: [{ aid: sId, bet: 0 }] };
                ...
                if (t === 'db') {
                    ...
                    gameState[roomId].oMp = dO.oddsMap; 
                    gameState[roomId].oS = dO.oddsStr; 
            ...
            if (body.match(/(^|\n)\/join\s+(chouhan|cc|derby)/) && gambleActive && gameState[roomId]?.state === 'REC') {
                if (!gameState[roomId].players.find(x => x.aid === sId)) { 
                    gameState[roomId].players.push({ aid: sId, bet: 0 }); 
                    sendMsg(roomId, `[info]🙋‍♂️ [piconname:${sId}] が参加しました！ (現在 ${gameState[roomId].players.length}人)[/info]`); 
                }
                return;
            }

            if (body.match(/(^|\n)\/start(chouhan|cc|derby)/) && gambleActive && gameState[roomId]?.state === 'REC' && gameState[roomId].host === sId) {
                if (gameState[roomId].players.length < 2) return sendTempMessage(roomId, `[info]⚠️ 参加者が2人以上でないと開始できません。[/info]`);
                clearTimeout(gameState[roomId].tid); 
                handleGameTimeout(roomId); 
                return;
            }

            if (body.trim() === '/leave' && gambleActive && gameState[roomId]) {
                let idx = gameState[roomId].players.findIndex(p => p.aid === sId);
                if (idx !== -1) {
                    let p = gameState[roomId].players[idx]; 
                    gameState[roomId].players.splice(idx, 1);
                    if (p.bet > 0) await addMoney(sId, p.bet); // 返金
                    
                    sendTempMessage(roomId, `[info]🚪 [piconname:${sId}] が退出しました。[/info]`);
                    if (gameState[roomId].players.length === 0) { 
                        clearTimeout(gameState[roomId].tid); 
                        if (gameState[roomId].rt) clearTimeout(gameState[roomId].rt);
            ...
            if (bM && gambleActive && gameState[roomId]?.state === 'BET') {
                let pl = gameState[roomId].players.find(x => x.aid === sId);
                if (pl && pl.bet === 0) {
                    let b = bM[2] === 'max' ? myMoney : (bM[2] === 'half' ? Math.floor(myMoney/2) : parseInt(bM[2], 10));
                    if (b > 0 && myMoney >= b) {
                        if (gameState[roomId].type === 'db') {
                            let h = bM[3]; 
                            if (!h || !gameState[roomId].oMp[h]) return sendTempMessage(roomId, `[info]⚠️ 馬連(例: 1-2)を正しく指定してください\n例: [code]/bet 100 1-2[/code][/info]`);
                            pl.choice = h;
                        }
                        pl.bet = b; 
                        await supabase.from('players').update({ money: myMoney - b }).eq('account_id', sId);
                        sendTempMessage(roomId, `[info]💰 [piconname:${sId}] ${fNum(b)} コインをベットしました！[/info]`);
                        checkGameProgress(roomId);
                    } else sendTempMessage(roomId, `[info]⚠️ ${mkRp(sId, roomId, msgId)} お金が足りません！[/info]`);
                }
                return;
            }

            if ((body.trim() === '/chou' || body.trim() === '/han') && gambleActive && gameState[roomId]?.type === 'ch' && gameState[roomId].state === 'ACT') {
                let pl = gameState[roomId].players.find(x => x.aid === sId);
                if (pl && !pl.choice) { 
                    pl.choice = body.trim().slice(1); 
                    sendTempMessage(roomId, `[info]🎯 [piconname:${sId}] 「${pl.choice==='chou'?'丁(偶数)':'半(奇数)'}」を選択しました！[/info]`); 
                    checkGameProgress(roomId); 
                }
            }

            if (body.trim() === '/roll' && gambleActive && gameState[roomId]?.type === 'cc' && gameState[roomId].state === 'ACT') {
                let pl = gameState[roomId].players.find(x => x.aid === sId);
                if (pl && !pl.res && sId !== gameState[roomId].host) {
                    pl.res = getChinchiroRoll(); 
                    sendMsg(roomId, `[info]🎲 [piconname:${sId}] の出目: ${pl.res.n}[/info]`); 
                    checkGameProgress(roomId);
                }
            }
