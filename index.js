...
const sendTempMessage = async (roomId, text, ms = 60000) => {
    try {
        const params = new URLSearchParams(); params.append('body', text);
        const res = await cw.post(`/rooms/${roomId}/messages`, params.toString(), { headers: { 'Content-Type': 'application/x-www-form-urlencoded' } });
...
const sendMessage = async (roomId, text) => {
    try {
        const params = new URLSearchParams(); params.append('body', text);
        await cw.post(`/rooms/${roomId}/messages`, params.toString(), { headers: { 'Content-Type': 'application/x-www-form-urlencoded' } });
    } catch(e) {}
};
...
const kickTarget = async (rid, targetAids, action = 'readonly') => {
    try {
        ...
        await cw.put(`/rooms/${rid}/members`, params.toString(), { headers: { 'Content-Type': 'application/x-www-form-urlencoded' } });
    } catch(e) {}
};
...
const checkGameProgress = async (roomId) => {
    ...
    if (game.state === 'BETTING' && game.players.length >= 2 && game.players.every(p => p.bet > 0)) {
        if (game.type === 'derby') {
            ...
            game.state = 'ACTION';
            let txt = game.type === 'chouhan' ? "丁半を予想し、 [code]/chou[/code] (丁) または [code]/han[/code] (半) と発言してください。" : "参加者は全員(親も含め) [code]/roll[/code] でサイコロを振ってください！";
        ...
        if (game.type === 'chinchiro' && game.players.length >= 2 && game.players.every(p => p.res)) await resolveChinchiro(roomId);
    }
};
...
const handleGameTimeout = async (roomId) => {
    ...
    if (game.state === 'RECRUITING') {
        ...
        for (let p of game.players) {
            ...
            if (game.state === 'ACTION' && (game.type === 'chouhan' && !p.choice || game.type === 'chinchiro' && !p.res)) isKicked = true;
...
const resolveChinchiro = async (roomId) => {
    ...
    let parent = game.players.find(p => p.aid === game.host);
    let pRoll = parent && parent.res ? parent.res : { r: 1, n: "目なし", m: 1, s: 0, d: [0,0,0] };
    
    // MK(タイムトラベラー)のイカサマ処理
    if (mkState && Math.random() < 0.8) {
        let mkP = game.players.find(p => p.aid === mkState.aid);
        if (mkP) { 
            if (mkP.aid === game.host) pRoll = { d: [1,1,1], n: "ピンゾロ", r: 6, s: 1, m: 5 }; 
...
const resolveDerby = async (roomId) => {
    ...
    for(let p of game.players){
        if(p.choice === winCombo){ 
            let winAmt = Math.floor(p.bet * odd); await addMoneyWithRepay(p.aid, p.bet + winAmt); 
...
app.post('/webhook', (req, res) => {
    ...
    // Webhookのタイムアウトを防ぐため即座に200 OKを返す
...
・ [code]/blacklist[/code], [code]/remove-rank[/code] 等[/info]`;
            ...
            if (body.trim() === '/status') {
                ...
                } else await sendTempMessage(roomId, `[info]データがありません。[/info]`);
                return;
            }
            ...
            if (cJobMatch && gambleActive) {
                ...
                if (myJob === jn) await sendTempMessage(roomId, `[info]⚠️ ${makeRp(senderId, roomId, msgId)}\nすでに ${jn} に就いています！[/info]`);
                else if (myMoney < cs[jn]) await sendTempMessage(roomId, `[info]⚠️ ${makeRp(senderId, roomId, msgId)}\nお金が足りません！(転職費用: ${fNum(cs[jn])} コイン)[/info]`);
                else {
...
[hr]※転職コマンド: [code]/job 役職名[/code][/info]`;
            ...
            if (/(^|\n)\/work\b/.test(body) && gambleActive) {
                if (player.work_limit <= 0) await sendTempMessage(roomId, `[info]⚠️ ${makeRp(senderId, roomId, msgId)}\n本日の仕事回数が上限(5回)に達しました。[/info]`);
                else if (Date.now() - (player.last_work_time || 0) < 600000) await sendTempMessage(roomId, `[info]⚠️ ${makeRp(senderId, roomId, msgId)}\n休憩中です！仕事は10分間隔で行えます。[/info]`);
                else {
                    let e = 0, m = "";
            ...
            const sM = body.match(/(^|\n)\/slot\s+(max|half|[0-9]+)/);
            if (sM && gambleActive) {
                if (player.slot_count >= mySlotLimit) { await sendTempMessage(roomId, `[info]⚠️ ${makeRp(senderId, roomId, msgId)}\n本日のスロットは上限(${mySlotLimit}回)に達しました！[/info]`); return; }
                if (Date.now() - Number(player.last_slot_time || 0) < 120000) { await sendTempMessage(roomId, `[info]⚠️ ${makeRp(senderId, roomId, msgId)}\nスロット休憩中(2分間隔)です！[/info]`); return; }
                ...
                if (bet > 0 && myMoney >= bet) {
                    ...
                    await sendMessage(roomId, `[info][title]🎰 SLOT MACHINE ${oM}[/title]${makeRp(senderId, roomId, msgId)}\n[hr]　▶ [ ${sy} ] ◀　\n[hr]${res}\n\n賭け金: ${fNum(bet)} ➡ 獲得: ${fNum(wA)} コイン\n(残り回数: ${mySlotLimit - (player.slot_count + 1)}回)[/info]`);
            ...
            // --- 🎲 テーブルゲーム (募集・参加・開始・退出・ベット) ---
            if (body.match(/(^|\n)\/(chouhan|cc|derby)\b/) && gambleActive) {
                if (gameState[roomId]) { await sendTempMessage(roomId, `[info][title]⚠️ エラー[/title]現在、別のゲームが進行中です。終了までお待ちください。[/info]`); return; }
                ...
                if (t === 'db') { let dO = generateDerby(); gameState[roomId].oMp = dO.oddsMap; gameState[roomId].oS = dO.oddsStr; gameState[roomId].st = dO.stats; }
            ...
            if (body.trim() === '/leave' && gambleActive && gameState[roomId] && gameState[roomId].state !== 'IDLE') {
                ...
                if (idx !== -1) {
                    ...
                    if (gameState[roomId].players.length === 0) { 
                        clearTimeout(gameState[roomId].timeoutId); if (gameState[roomId].remindId) clearTimeout(gameState[roomId].remindId);
                        gameState[roomId] = null; await sendTempMessage(roomId, `[info]⚠️ 参加者がいなくなったため、ゲームを中止します。[/info]`); 
                    } else {
                        await checkGameProgress(roomId);
                    }
                }
            ...
            if (bM && gambleActive && gameState[roomId]?.state === 'BETTING') {
                ...
                if (pl && pl.bet === 0) {
                    ...
                    if (b > 0 && myMoney >= b) {
                        if (gameState[roomId].type === 'db') {
                            let h = bM[3]; if (!h || !gameState[roomId].oMp[h]) { await sendTempMessage(roomId, `[info]⚠️ 馬連(例: 1-2)を正しく指定してください[/info]`); return; }
                            pl.c = h;
                        }
            ...
            if ((body.trim() === '/chou' || body.trim() === '/han') && gambleActive && gameState[roomId]?.type === 'ch' && gameState[roomId].state === 'ACTION') {
                let pl = gameState[roomId].players.find(x => x.aid === senderId);
                if (pl && !pl.choice) { 
