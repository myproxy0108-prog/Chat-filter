...
[hr]※転職コマンド: /#job 役職名[/info]`);
            ...
            if (/(^|\n)[/#]work\b/.test(body) && gambleActive) {
                ...
                else if(myJob === 'Terminal'){ e=Math.floor(Math.random()*2501)+1500; m=`不正アクセスで裏帳簿を書き換え、 ${formatNumber(e)} コイン稼ぎました！🖥️`; }
                else if(myJob === 'Photographer'){ e=Math.floor(Math.random()*2501)+1500; m=`決定的瞬間を切り取って売り、 ${formatNumber(e)} コイン稼ぎました！📷`; }
                // アカウント単位でロック＋再取得し、job_stateへの競合書き込みによる実績の取りこぼしを防止
                let workTotal = 0;
                await withBadgeLock(senderId, async () => {
