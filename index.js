...
const processBuffs = async (aid, isWin, isLose, isDraw, mult, resTxt, betAmount = 0) => {
    ...
    // 【Fixer】副能力『役職装備』: 装着した役職のスキルを弱体化した形でパッシブ発動する
    if (js.fix_equipped_job && isWin) {
        let jb = JOB_PASSIVE_MULT[js.fix_equipped_job] || 0;
        if (jb > 0) {
            mult += jb;
            resTxt += `\n🕴️🎓 【装備:${js.fix_equipped_job}】の力が配当を押し上げた (+${jb}倍)`;
        }
    }
...
const processBounty = async (loserAid, lostAmount, roomId) => {
    ...
    if (hunters) {
        for (let h of hunters) {
            ...
            if (js.bounty_target === loserAid.toString() && !js.daily_bounty_used) {
                let pct = js.evo_boost_bounty_pct || 0.1;
        ...
        if (gokudoPlayers) {
            ...
            if (player && player.last_daily_date !== today) {
                ...
                if (diff === 1) {
                    ...
                    // 【たかしくん】ログインし忘れても、生涯3回まで連続記録を守ってくれる(4回目以降は普通にリセットされる)
                ...
                player.job_state.fix_equip_date = null; // 装備の1日1回制限をリセット(装備自体は外れない)
                ...
                // 【時は金なり】利息(銀行員+黄金の招き猫)で獲得した金額の累計
                ...
                // 【トレーダー】保有資産(株)から毎日配当金
                if (player.job === 'トレーダー') {
                    let stockVal = (player.kabu_owned || 0) * kabuData.price;
                    let myStocksForDiv = player.stocks ? (typeof player.stocks === 'string' ? JSON.parse(player.stocks || '{}') : player.stocks) : {};
                ...
                // 10%の確率(【占い師】は必ず)で、まだ見つけていない実績(？？？)のヒントを発見する
                let hintMsg = "";
...
[hr]※転職コマンド: /#job 役職名[/info]`);
            ...
            if (/(^|\n)[/#]equip\b/.test(body) && gambleActive) {
                ...
                if (!eqArg) {
                    ...
                    let jobList = Object.entries(JOB_PASSIVE_MULT).map(([k, v]) => `・${k} (勝利時 配当+${v}倍)`).join('\n');
                ...
                if (EQUIP_ITEM_EFFECTS[eqArg]) {
                    ...
                    await supabase.from('players').update({ job_state: JSON.stringify(player.job_state) }).eq('account_id', senderId);
                    return sendMessage(roomId, `[info][title]🕴️ 装備完了 -【${eqArg}】[/title][piconname:${senderId}]\nその肩書き、俺が使わせてもらう。\n▶ 勝利時に配当 +${JOB_PASSIVE_MULT[eqArg]}倍 (弱体化された常時発動)[/info]`);
                } else {
            ...
            if (evolveMatch && gambleActive) {
                ...
                const getStockValue = async (aid) => {
                    ...
                    let myStocks = sv?.stocks ? (typeof sv.stocks === 'string' ? JSON.parse(sv.stocks || '{}') : sv.stocks) : {};
            ...
            if (/(^|\n)[/#]work\b/.test(body) && gambleActive) {
                ...
                if(myJob === 'サラリーマン'){ if(Math.random() < 0.1){ e=0; m="仕事で重大なミスをしてしまい、本日の給料は 0 コインに...😭"; } else { e=Math.floor(Math.random()*1601)+400; m=`真面目に働き、 ${formatNumber(e)} コイン稼ぎました！💼`; } }
                    ...
                    e=Math.floor(Math.random()*1601)+1200; 
