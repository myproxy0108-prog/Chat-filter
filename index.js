...
// --- 【修正】Chatwork公式準拠の署名検証ロジック ---
const verifySignature = (req) => {
    const signature = req.headers['x-chatworkwebhooksignature'];
    if (!signature || !req.rawBody || !WEBHOOK_TOKEN) return false;
    
    // トークン自体はBase64エンコードされた秘密鍵としてHMACを生成する
...
const resolvePoker = async (rid) => {
    ...
    } catch(e) { console.error(e); } finally { gameState[rid] = null; }
};
...
app.post('/webhook', async (req, res) => {
    ...
    // 【最重要修正】Vercelによる非同期プロセスの途中消滅を防ぐため、このハンドラー内ですべての処理を完結させてから、関数の末尾でOKを返す設計に変更
    const ev = req.body.webhook_event;
    if (!ev || ev.webhook_event_type !== 'message_created') {
    ...
    try {
        ...
        // 1. ブラックリスト防衛 (maybeSingleを使うことで、存在しなくてもエラーにならず安全に次へ進む)
        ...
        if (body.trim() === '/help-gya') {
            const h = `[info][title]🎰 カジノ＆ライフ 総合案内 (V42 PERFECT FINAL)[/title]
【 🏦 銀行・ステータス 】
...
[hr]※転職コマンド: [code]/job 役職名[/code][/info]`;
        ...
        if (/(^|\n)\/(catch|goal|slot-up|cm|MK|KK)\b/.test(body) && gambleActive) {
            ...
            if (cmd === 'goal' && myJob !== 'pro') return res.status(200).send('OK');
            ...
            if (cmd === 'catch') {
                ...
                let now = Date.now(), targets = betLogs.filter(l => now - l.t <= 300000), diffs = {};
        ...
        if (sM && gambleActive) {
            ...
            if (body.match(/(^|\n)\/start(chouhan|cc|derby)/) && gambleActive && gameState[roomId]?.state === 'RECRUITING' && gameState[roomId].host === senderId) {
                if (gameState[roomId].players.length < 2) { await sendTempMessage(roomId, `[info]⚠️ 参加者が2人以上でないと開始できません。[/info]`); return res.status(200).send('OK'); }
