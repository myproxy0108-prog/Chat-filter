...
const spamRecords = {};
const gameState = {}; // 全ゲームの状態管理

// 起動時にギャンブル状態を取得
supabase.from('config').select('value').eq('key', 'gamble_active').single()
    .then(r => { if(r.data) gambleActive = r.data.value === 'true'; })
    .catch(()=>{});
...
const sendMessage = (roomId, text) => {
    cw.post(`/rooms/${roomId}/messages`, `body=${encodeURIComponent(text)}`).catch(()=>{});
};

const deleteMessage = (roomId, msgId) => {
    cw.delete(`/rooms/${roomId}/messages/${msgId}`).catch(()=>{});
};
...
const checkGameProgress = async (roomId) => {
    ...
    if (game.state === 'BETTING' && game.players.length >= 2 && game.players.every(p => p.bet > 0)) {
        ...
        if (game.type === 'chouhan' && game.players.length >= 2 && game.players.every(p => p.choice)) {
            await resolveChouhan(roomId);
        }
        if (game.type === 'chinchiro' && game.players.length >= 2 && game.players.filter(x => x.aid !== game.host).every(p => p.res)) {
    ...
    (async () => {
        try {
            ...
            if (gambleActive && player && !body.startsWith('/')) {
                ...
                if (mc >= (Math.floor(Math.random() * 21) + 30)) { 
                    mc = 0; if (wl < 10) wl++; 
                }
...
・ [code]/blacklist[/code], [code]/remove-rank[/code] 等[/info]`;
            ...
            if (body.trim() === '/status') {
                ...
                return sendTempMessage(roomId, `[info][title]📊 プレイヤー情報[/title][piconname:${senderId}]\n💰 所持金: ${fNum(myMoney)} コイン\n💳 借金: -${fNum(myDebt)} コイン\n💎 純資産: ${fNum(myMoney - myDebt)} コイン\n[hr]👔 職業: ${myJob}\n🎰 スロット残り: ${Math.max(0, 3 - (player?player.slot_count:0))} 回\n💼 お仕事残り: ${player?player.work_limit:0} 回${omi}[/info]`);
            }
...
[hr]※転職コマンド: [code]/job 役職名[/code][/info]`);
            ...
            if ((/(^|\n)\/catch\b/.test(body) || /(^|\n)\/goal\b/.test(body)) && gambleActive && player) {
                ...
                if (iC) { if(sc){ e=800; m=`見事犯人を逮捕しました！\n特別報酬 ${e} コイン獲得！🚨`; } else m=`犯人を逃してしまいました...🏃‍♂️💨`; }
                else { if(sc){ e=1000; m=`スーパーゴールを決めました！\nスポンサーから ${e} コイン獲得！🥅✨`; } else m=`シュートは外れてしまいました...🤦‍♂️`; }
            ...
            if (body.match(/(^|\n)\/(chouhan|cc|derby)\b/) && gambleActive) {
                ...
                if (gCD) return sendTempMessage(roomId, `[info]⚠️ ゲームは3分間隔です。もう少しお待ちください。[/info]`);
