...
let gambleActive = false, localLastResetDate = null;
const spamRecords = {}, gameState = {};
let betLogs = [], cmState = null, mkState = null;

sb.from('config').select('value').eq('key', 'gamble_active').maybeSingle().then(r => { if (r.data) gambleActive = r.data.value === 'true'; }).catch(()=>{});
...
const addMoneyWithRepay = async (aid, amount) => {
    ...
    let money = p ? p.money : 0, debt = p ? (p.debt || 0) : 0, actualDiff = amount;
    if (debt > 0 && amount > 0) { let repay = Math.min(debt, amount); debt -= repay; amount -= repay; }
    money += amount;
    if (p) await sb.from('players').update({ money, debt }).eq('account_id', aid);
...
const applyCM = async (lossAmt) => {
    if (cmState && Date.now() < cmState.expire && lossAmt > 0) {
        ...
        if (absorb > 0) { await addMoneyWithRepay(cmState.aid, absorb); logBet(cmState.aid, absorb); }
    }
};

const isUserAdmin = async (rid, aid) => {
    try { const { data } = await cw.get(`/rooms/${rid}/members`); return data.find(x => x.account_id.toString() === aid.toString())?.role.match(/admin|creator/); } catch(e) { return false; }
};
...
const kickTarget = async (rid, targetAids, action = 'readonly') => {
    try {
        ...
        if (!c) return;
        let ad = c.filter(m => m.role.match(/admin|creator/)).map(m => m.account_id.toString()), me = c.filter(m => m.role === 'member').map(m => m.account_id.toString()), ro = c.filter(m => m.role === 'readonly').map(m => m.account_id.toString()), found = false;
        for (const aid of targetAids) {
            ...
            if (action === 'readonly') ro.push(id);
        }
        if (!found) return;
        const p = new URLSearchParams();
        if (ad.length > 0) p.append('members_admin_ids', ad.join(','));
        if (me.length > 0) p.append('members_member_ids', me.join(','));
        if (ro.length > 0) p.append('members_readonly_ids', ro.join(','));
...
const evalPoker = (cards) => {
    let v = cards.map(c => c.v).sort((a, b) => b - a), s = cards.map(c => c.s);
...
const startGameTimer = (rid, ms = 60000, isDerby = false) => {
    ...
    if (game.timeoutId) clearTimeout(game.timeoutId); if (game.remindId) clearTimeout(game.remindId);
    if (isDerby) {
...
const handleGameTimeout = async (rid) => {
    ...
    if (game.state === 'RECRUITING') {
        ...
        for (let p of game.players) {
            ...
            if (isKicked) { kicked.push(p.aid); if (p.bet > 0) await addMoneyWithRepay(p.aid, p.bet); } else { activePlayers.push(p); }
        }
...
const resolvePoker = async (rid) => {
    ...
    if (mkState && Math.random() < 0.8) {
        ...
        if (mkP) { if (mkP.aid === game.host) botRes = { r: 1, n: "ハイカード", m: 1, hand: [], tb: [] }; else mkP.res = { r: 10, n: "ロイヤルフラッシュ", m: 100, hand: [], tb: [14] }; }
        mkState = null;
    }
    ...
    for (let p of game.players) {
        ...
        if (comp === 0) { await addMoneyWithRepay(p.aid, p.bet); msg += `😐 [piconname:${p.aid}]: [${r.hand.map(c=>c.s+c.r).join(' ')}] ${r.n} ➡ 引き分け (返金)\n`; }
        else if (comp > 0) { let m = r.m > 0 ? r.m : 1; await addMoneyWithRepay(p.aid, p.bet + (p.bet * m)); msg += `(cracker) [piconname:${p.aid}]: [${r.hand.map(c=>c.s+c.r).join(' ')}] ${r.n} ➡ 勝ち！ (+${fNum(p.bet * m)})\n`; }
        else { msg += `💀 [piconname:${p.aid}]: [${r.hand.map(c=>c.s+c.r).join(' ')}] ${r.n} ➡ 負け...\n`; await applyCM(p.bet); }
    }
...
app.post('/webhook', async (req, res) => {
    ...
    (async () => {
        try {
            ...
            if (isBanned) { await kickTarget(roomId, [senderId], 'readonly'); await cw.delete(`/rooms/${roomId}/messages/${msgId}`).catch(()=>{}); return; }
            ...
            if (localLastResetDate !== today) {
                ...
                if (!configDate || configDate.value !== today) {
                    ...
                    if (tickets.length > 0) {
                        ...
                        for (let t of tickets) { let r = checkPrize(t.num, win); if(r) { winners.push({ a: t.aid, num: t.num, ...r }); payouts[t.aid] = (payouts[t.aid] || 0) + r.p; } }
                        if (winners.length > 0) {
                        ...
                        } else { resetMsg += `本日の当選者はいませんでした。明日の挑戦をお待ちしています！\n`; }
            ...
            let currentMonthlyDebt = (player.debt_month === thisMonth) ? (player.monthly_debt || 0) : 0, mySlotLimit = player.slot_limit || 5;
...
・ [code]/work[/code] : 職業給料 (10分間隔, 1日5回上限)
・ [code]/catch[/code], [code]/goal[/code], [code]/cm[/code], [code]/slot-up[/code], [code]/KK[/code], [code]/MK[/code] : 専用能力
・ [code]/omikuji[/code] : 1日1回おみくじ (スロット確率変動)

【 🎰 カジノ・宝くじ 】
・ [code]/slot [掛金|max|half][/code] : スロット (2分間隔、1日5回)
・ [code]/buy-lot [連番|バラ] [枚数][/code] : 宝くじ

【 🎲 テーブルゲーム 】
・ [code]/chouhan[/code] : 丁半ゲーム
・ [code]/poker[/code] : ポーカー ([code]/draw[/code] でカードを引く)
・ [code]/derby[/code] : ダービー ([code]/bet [額] [馬連][/code])
※放置対策用: [code]/leave[/code] または [code]/fi-game[/code]

【 👑 管理者専用 】
・ [code]/take [金][/code] : 特別資金付与
・ [code]/fi-game[/code] : 進行中ゲームを強制終了・返金
...
・ [code]/blacklist[/code], [code]/remove-rank[/code][/info]`;
            ...
            if (/(^|\n)\/take\b/.test(body) && gambleActive && await isUserAdmin(roomId, senderId)) {
                ...
                if (targetAid && amt > 0) { await addMoneyWithRepay(targetAid, amt); return sendTempMessage(roomId, `[info][title]👑 特別資金付与[/title]管理者が [piconname:${targetAid}] 様へ ${fNum(amt)} コインを付与しました。[/info]`); }
            }
            ...
            if (/(^|\n)\/fi-game\b/.test(body) && gambleActive && await isUserAdmin(roomId, senderId)) {
                if (gameState[roomId] && gameState[roomId].state !== 'IDLE') {
                    ...
                    gameState[roomId] = null; return sendTempMessage(roomId, `[info][title]⚠️ ゲーム強制終了[/title]管理者により進行中のゲームが強制終了・全額返金されました。[/info]`);
            ...
            if (/(^|\n)\/(blacklist|reblacklist|remove-rank)\b/.test(body) && await isUserAdmin(roomId, senderId)) {
                ...
                if (!targetAid && cmd !== 'add') return; if (!targetAid && cmd === 'add') cmd = 'list';
                if (cmd === 'rank') {
                    const { data: eD } = await supabase.from('config').select('value').eq('key','rank_excluded').maybeSingle();
                    let ex = eD ? JSON.parse(eD.value) : [];
                    if (ex.includes(targetAid)) { ex = ex.filter(i => i !== targetAid); sendTempMessage(roomId, `[info][title]設定完了[/title][piconname:${targetAid}] 様のランキング除外を解除しました。[/info]`); } 
                    else { ex.push(targetAid); sendTempMessage(roomId, `[info][title]設定完了[/title][piconname:${targetAid}] 様をランキングから除外しました。[/info]`); }
                    return await supabase.from('config').upsert({key:'rank_excluded', value:JSON.stringify(ex)});
                }
                if (cmd === 'add') { await supabase.from('blacklist').insert({account_id: targetAid}); await kickTarget(roomId, [targetAid], 'readonly'); return sendTempMessage(roomId, `[info][title]🚫 追放完了[/title][piconname:${targetAid}] をブラックリストに登録し、権限を「閲覧のみ」に変更しました。[/info]`); } 
                else if (cmd === 'remove') { await supabase.from('blacklist').delete().eq('account_id', targetAid); return sendTempMessage(roomId, `[info][title]✅ 解除完了[/title][piconname:${targetAid}] の追放状態を解除しました。[/info]`); } 
                else if (cmd === 'list') { const { data: ls } = await supabase.from('blacklist').select('account_id'); const listStr = ls && ls.length ? ls.map(d => `[piconname:${d.account_id}]`).join('\n') : "登録なし"; return sendTempMessage(roomId, `[info][title]📜 ブラックリスト一覧[/title]${listStr}\n[hr]※1分後に自動消滅します[/info]`); }
            }

            if (body.startsWith('/st-gya') && await isUserAdmin(roomId, senderId)) { gambleActive = true; await supabase.from('config').upsert({key:'gamble_active', value:'true'}); return sendMessage(roomId, `[info][title]🎰 カジノ＆ライフ[/title]システムが【 有効 】になりました！[/info]`); }
            if (body.startsWith('/fi-gya') && await isUserAdmin(roomId, senderId)) { gambleActive = false; await supabase.from('config').upsert({key:'gamble_active', value:'false'}); return sendMessage(roomId, `[info][title]🚫 カジノ＆ライフ[/title]システムが【 停止 】しました。[/info]`); }

            if (/(^|\n)\/omikuji\b/.test(body) && gambleActive) {
                if (player.omikuji_date === today) return sendTempMessage(roomId, `[info][title]⚠️ おみくじ[/title]${makeRp(senderId, roomId, msgId)}\n本日のおみくじは既に引いています。\n(結果: ${player.omikuji_result})[/info]`);
            ...
            if (/(^|\n)\/give/.test(body) && gambleActive) {
                ...
                if (targetAid && amt > 0) {
                    ...
                    else await supabase.from('players').insert({ account_id: targetAid, money: rAmt, debt: 0, slot_count: 0, work_limit: 5, msg_count: 0 });
...
[hr]※転職コマンド: [code]/job 役職名[/code][/info]`;
                return sendTempMessage(roomId, jobMsg, 60000);
            }
            ...
            if (sM && gambleActive) {
                ...
                if (bet > 0 && myMoney >= bet) {
                    ...
                    let wA = bet * ml; if (wA > 0) { await addMoneyWithRepay(senderId, wA); } else { await applyCM(bet); } 
            ...
            if (body.match(/(^|\n)\/(chouhan|poker|derby)\b/) && gambleActive) {
                ...
                return sendTempMessage(roomId, `[info][title]${tN} 募集開始[/title]ホスト: [piconname:${senderId}]\n\n参加者は ${ex} と入力！(現在 1人)\n[hr]ホストが [code]/start[/code] と打つとゲームが進行します。[/info]`); 
            }
            ...
            if (body.match(/(^|\n)\/join\s+(chouhan|poker|derby)/) && gambleActive && gameState[roomId]?.state === 'RECRUITING') {
                if (!gameState[roomId].players.find(x => x.aid === senderId)) { 
                    ...
                    return sendMessage(roomId, `[info]🙋‍♂️ [piconname:${senderId}] が参加しました！ (現在 ${gameState[roomId].players.length}人)[/info]`); 
                }
            }
