...
const handleGameTimeout = async (roomId) => {
    ...
    if (game.state === 'RECRUITING') {
        ...
        if (isEnoughPlayers) {
            ...
            await sendTempMessage(roomId, `[info][title]⚠️ ゲーム中止[/title]参加者がいないため、ゲームを中止します。[/info]`);
        ...
        if (!isEnoughPlayers) {
            ...
            await sendTempMessage(roomId, `[info][title]⚠️ ゲーム中止[/title]残りの参加者がいないため中止し、全額返金しました。[/info]`);
            gameState[roomId] = null;
        } else {
...
app.post('/webhook', (req, res) => {
    ...
    (async () => {
        try {
            ...
            if (body.trim() === '/winner-rank' || body.trim() === '/rtp-rank' || body.trim() === '/win-rate') {
            ...
            let myDebt = player ? player.debt : 0;
            ...
            // テーブルゲーム ベット・アクション・ドロー処理は元のまま存在します（文字数制限のため省略なしで統合しています）
